...
import com.butel.connect.LogUtil;
...
private final static String TAG="RadarDemo";
	//界面空间相关
	...
	private Button uploadOnece;//获取当前位置并上传
	private Button uploadContinue;//连续获得位置并上传
	private Button stopUpload;//停止上传
	private Button switchBtn;//地图
	private Button searchNearbyBtn;//搜索周边

	private Button clearRstBtn;//	清除结果
	private Button clearInfoBtn;//清除当前信息
	private int index = 0;
...
//		userID="999";
		...
		Log.d(TAG,"Create () onuserID="+userID);
		// 定位初始化
	...
	public void uploadOnceClick(View v) {
		...
		info.pt = pt;
	boolean uploadResult=	RadarSearchManager.getInstance().uploadInfoRequest(info);
		Log.d("slk","uploadInfoRequest result="+uploadResult);
	...
	public void onGetNearbyInfoList(RadarNearbyResult result,
		...
		if (error == RadarSearchError.RADAR_NO_ERROR) {
			...
			.show();
			//获取成功
			LogUtil.d("获取周边信息list成功");
			listResult = result;
			if(listResult.infoList.size()>=1){
				List <RadarNearbyInfo> mylist=listResult.infoList;
				RadarNearbyInfo info=mylist.get(0); 
				LogUtil.d("附近第一个RadarNearbyInfo:" +
						"用户ID["+info.userID+"] , " +
						"备注信息["+info.comments+"] , " +
						"用户坐标[维度"+info.pt.latitude+"|经度"+info.pt.longitude+"] , " +
						"距离["+info.distance+"] , " +
						"最后更新时间戳["+info.timeStamp+"] , " +
						"设备名称["+info.mobileName+"] , " +
						"设备系统名称["+info.mobileOS+"]" +
								".");
			}
	...
	public void onGetUploadState(RadarSearchError error) {
		Log.d("RadarDemo", "onGetUploadState error="+error);
		if (error == RadarSearchError.RADAR_NO_ERROR) {
			//上传成功
			if (!uploadAuto) {
